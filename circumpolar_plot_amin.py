...
def circumpolar_plot (file_path, var_name, depth_key, depth, depth_bounds, colour_bounds=None, save=False, fig_name=None, grid_path=None):
...
if __name__ == "__main__":
    ...
    if len(id.variables[var_name].shape) == 4:
        ...
        if depth_type == 's':
            depth_input = input("Surface layer (s), bottom layer (b), or specific depth (d)? ")
            if depth_input == 's':
                depth_key = 0
                depth = NaN
                depth_bounds = None
                ...
                depth_key = 2
                depth = -1*float(input("Enter depth (positive, in metres): "))
                depth_bounds = None
        elif depth_type == 'v':
    ...
    # Make the plot
    circumpolar_plot(file_path, var_name, depth_key, depth, depth_bounds, colour_bounds, save, fig_name, grid_path)
    ...
    while True:
        ...
        if repeat == 'y':
            while True:
                ...
                changes = input("Enter a parameter to change: (1) file path, (2) variable name, (3) depth, (4) colour bounds, (5) save/display; or enter to continue: ")
                if len(changes) == 0:
