...
def plot_vslice (file_path, timestep, variable, depth_min, depth_max, i_min, j_min, i_max, j_max, Vstretching, theta_s, theta_b, hc, N):
...
if __name__ == "__main__":
    ...
    timestep = int(input("Time step to plot: "))
    ...
    N = 31#int(input("N: "))
    plot_vslice (file_path, timestep, variable, depth_min, depth_max, i_min, j_min, i_max, j_max, Vstretching, theta_s, theta_b, hc, N)
    ...
    while True:
        ...
        if repeat == 'y':
            while True:
                changes = input("Enter a parameter to change: (1) file, (2) time step, (3) variable, (4) depth, (5) start or end coordinate; or enter to continue: ")
                if len(changes) == 0:
                    break
                else:
                    if int(changes) == 1:
                        grid_path = input("Path to file: ")
                    if int(changes) == 2:
                        grid_path = input("Time step: ")
                    if int(changes) == 3:
                        grid_path = input("Variable: ")
                    elif int(changes) == 4:
                        depth_min = -1*float(input("Deepest depth to plot (positive, metres): "))
                        depth_max = -1*float(input("Shallowest depth to plot (positive, metres): "))
                    elif int(changes) == 5:
