...
def timeseries_3D (grid_path, file_path, log_path):
    ...
    while True:
        ...
        for t in range(num_time_curr):
            ...
            # but that's ok
            dx, dy, dz_tmp, z = cartesian_grid_3d(lon, lat,lon_u,lat_u,lon_v,lat_v, h, zice, theta_s, theta_b, hc, N, zeta[t,:,:])
            dz[t,:,:,:] = dz_tmp
        ...
        for t in range(num_time_curr):
            ...
            ohc_deep.append(sum((temp[t,index]+C2K)*rho[t,index]*Cp*dV[t,index]))
...
if __name__ == "__main__":
    ...
    file_path = input("Path to ROMS history/averages file: ")
    log_path = input("Path to logfile to save values and/or read previously calculated values: ")
